...
namespace Renderer
{
    ...
    void PutChar(char chr, STL::Point Pos, uint8_t Scale)
    {
        ...
        for (uint32_t y = 0; y < 16 * Scale; y++)
        {
            for (uint32_t x = 0; x < 8 * Scale; x++)
            {
    ...
    void ScrollUp(uint32_t Amount)
    {
        uint32_t Offset = Backbuffer.PixelsPerScanline * Amount;
    ...
    void RedrawMouse()
    {
        ...
        {            
            ...
            for (uint32_t y = OldMousePos.Y; y < OldMousePos.Y + 16; y++)
            {
                for (uint32_t x = OldMousePos.X; x < OldMousePos.X + 16; x++)
                {
            ...
            for (uint32_t y = Mouse::Position.Y; y < Mouse::Position.Y + 16; y++)
            {
                for (uint32_t x = Mouse::Position.X; x < Mouse::Position.X + 16; x++)
                {
                    *(STL::ARGB*)((uint64_t)Renderer::Frontbuffer->Base + x * 4 + y * Renderer::Frontbuffer->PixelsPerScanline * 4) = 
                    *(STL::ARGB*)((uint64_t)Renderer::Backbuffer.Base + x * 4 + y * Renderer::Backbuffer.PixelsPerScanline * 4);
                }
